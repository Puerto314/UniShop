 ...
 *
 * ✅ SEGURIDAD DE DATOS:
 *    - Todos los repositorios y servicios externos son MOCKS (Mockito).
 *    - El CorreoService es mockeado: NO se envían correos reales.
 *    - NUNCA se toca la base de datos real ni se envían correos reales.
...
public class ClienteServiceTest {
    ...
    void create_cuandoDatosValidos_retorna1() {
        ...
        // Verifica que se intentó enviar el correo (mock: no envía nada real)
    ...
    void create_cuandoCorreoArrobaAlInicio_retornaMenos2() {
        // '@' en posición 0 → arroba <= 0 → inválido
        when(clienteRepository.existsByNombreUsuario(NOMBRE)).thenReturn(false);
    ...
    void create_cuandoCorreoValido_seEnviaCorreoMock() {
        // Verifica la integración con CorreoService (mock, no envía nada real)
        when(clienteRepository.existsByNombreUsuario(NOMBRE)).thenReturn(false);
        when(clienteRepository.save(any(Cliente.class))).thenReturn(clienteMock);
        ...
        // El cliente igual se crea aunque el correo falle
